...
export class BooksService {
  ...
  async checkAvailability(book: any, user: any) {
    ...
    return await this.firestore
      .collection('prestamos', (ref) => ref.where('bookId', '==', book.id))
      .get()
      .subscribe((res) => {
  ...
  async checkUserAlreadyHasThisBook(
    book: any,
    user: any,
    sizePrestamos: number
  ) {
    //Check if the user has already rented this book
    return await this.firestore
      .collection('prestamos', (ref) =>
        ref.where('dniUSer', '==', user.dni).where('bookId', '==', book.id)
      )
      .get()
      .subscribe((res) => {
        if (res.size > 0) {
  ...
  async updateBookEntered(book: any) {
    ...
    await this.firestore
      .collectionGroup('librosIngresados', (ref) =>
        ref.where('id', '==', book.bookId)
      )
      .get()
      .subscribe((res) => {
